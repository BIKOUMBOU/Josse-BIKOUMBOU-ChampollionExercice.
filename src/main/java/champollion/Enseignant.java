...
public class Enseignant extends Personne {
    ...
    public int heuresPrevuesPourUE(UE ue) {// TODO: Implémenter cette méthode
		...
		if (p != null) { // La clé existe, l'enseignant intervient dans l'UE
			result = 
				p.getVolumehoCM() * 1.5f + p.getVolumehoTD() + p.getVolumehoTP() * 0.75f
			;
		}
    ...
    public void ajouteEnseignement(UE ue, int volumehoCM, int volumehoTD, int volumehoTP) { // TODO: Implémenter cette méthode
       	if (volumehoCM < 0 | volumehoTD < 0 | volumehoTP < 0) {
			throw new IllegalArgumentException("Les valeurs supérieures ou égales à 0");
		}
		...
		if (sp == null) { 
			...
			sp.setVolumehoCM(volumehoCM + sp.getVolumehoCM());
			sp.setVolumehoTD(volumehoTD + sp.getVolumehoTD());
			sp.setVolumehoTP(volumehoTP + sp.getVolumehoTP());
		}
    }
